...
data "aws_ami" "ubuntu_hirsute" {
  most_recent = true
  owners      = ["099720109477"]
  filter { name = "name" values = ["ubuntu/images/hvm-ssd/ubuntu-hirsute-21.04-amd64-server-*"] }
}
...
resource "aws_security_group" "demo" {
  ...
  ingress { from_port = 19999 to_port = 19999 protocol = "tcp" cidr_blocks = ["0.0.0.0/0"] }
...
resource "aws_instance" "frontend" {
  ...
  tags = { Name = "c8.local", Role = "frontend" }
...
resource "aws_instance" "backend" {
  ...
  tags = { Name = "u21.local", Role = "backend" }
